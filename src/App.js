...
function App() {
  ...
  const calcTfsa = (event) => {
    ...
    if (dob === "") {
      ...
      const filterLimit = tfsaArray.filter((item) => {
        return item.year >= birthYear + 18;
      });
      sum = filterLimit.reduce(
    ...
    <div className="flex h-screen justify-center items-center">
      <div class="rounded-xl shadow-2xl bg-white bg-opacity-90 p-3 border-2 text-center bg-gradient-to-r from-slate-300">
        <div className="text-4xl font-bold my-6">
          ...
          <span className="underline decoration-blue-700 decoration-4 underline-offset-4 italic">
            Calculator
          </span>
        </div>
        <form className="space-y-6 text-2xl">
          ...
          <div className="grid space-x-3 space-y-2 mx-4">
            <label className="ml-3 font-semibold text-left">
          ...
          <div className="space-x-5 ">
            <button
              className="bg-blue-700 hover:bg-blue-500 text-white font-bold py-3 px-6 rounded-xl shadow-2xl"
              onClick={calcTfsa}
              type="submit"
            >
              Submit
        ...
        <h3 className="opacity-75 grid text-5xl font-semibold rounded w-3/6 bg-center mx-auto h-24 place-content-center drop-shadow-xl">
          {limit}
        </h3>
